...
url_prefix_string = "http://www.tripadvisor.ca/Hotel_Review-g190479-d3587956-Reviews-or"
url_postfix_string = "-The_Thief-Oslo_Eastern_Norway.html#REVIEWS"
url_first = "https://www.tripadvisor.ca/Hotel_Review-g190479-d3587956-Reviews-The_Thief-Oslo_Eastern_Norway.html#REVIEWS"

WebSitesNew = []
for i in range(0, 97):
    website = url_prefix_string + str(i*10) + url_postfix_string
    WebSitesNew.append(website)

# List the first page of the reviews (ends with "#REVIEWS") - separate the websites with ,
WebSites = [
    url_prefix_string + '10' + url_postfix_string,  # page 2
    url_prefix_string + '50' + url_postfix_string,  # page 6
]


def crawlToCSV(theurl):
    page_number = str(int(theurl.replace(url_prefix_string,'').replace(url_postfix_string,''))/10+1)
    if theurl == url_first:
        page_number = '1'
    ...
    try_total = 10
    try_times = 0
    while not soup and try_times < try_total:
        try_times += 1
...
# extract the rating count for each user review
    ...
    for x in range(0, len(hotelarray)):
        ...
        if soup.findAll(attrs={"class": "reviewerBadge badge"}) and len(soup.findAll(attrs={"class": "reviewerBadge badge"})) > x:
            ...
            ReviewCount = '0'
        #
        if soup.findAll(attrs={"class": "member_info"}) and len(soup.findAll(attrs={"class": "member_info"})[x].findAll(attrs={"class": "location"})) > 0:
            Location = soup.findAll(attrs={"class": "member_info"})[x].findAll(attrs={"class": "location"})[0].text.replace(',', ' ').strip()
        else:
            Location = ''
...
if __name__=='__main__':
    ...
    with open("Windsor.csv", "wt", newline='', encoding="utf8") as f:
